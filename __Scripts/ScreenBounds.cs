...
public class ScreenBounds : MonoBehaviour
{
	...
	[Range(0,10)]
	[Tooltip("Adds to all directions of scale (does scaleColl + Vector3.one * extraScale).")]
	public float extraScale = 0;
    Camera cam;
    BoxCollider boxColl;
    ...
    private Vector3 CalculateScale()
    {
        ...
        return scaleColl + Vector3.one * extraScale;
	...
	static public Vector3 RANDOM_ON_SCREEN_LOC_HALF
	{
		get
		{
			Vector3 min = S.boxColl.bounds.min/2;
			Vector3 max = S.boxColl.bounds.max/2;
			Vector3 loc = new Vector3(Random.Range(min.x, max.x), Random.Range(min.y, max.y), 0);
			return loc;
		}
	}
